...
.DESCRIPTION
    Unprotect-String decrypts an encrypted string that was encrypted using Protect-String.
    
.PARAMETER EncryptedData
    The encrypted data object returned by Protect-String.
...
function Unprotect-String {
    ...
    try {
        if ($PSCmdlet.ParameterSetName -eq 'PSK') {
            $result = $script:CryptoProvider.DecryptString($EncryptedData, $Algorithm, $Password)
        }
        elseif ($PSCmdlet.ParameterSetName -eq 'Certificate') {
            $result = $script:CryptoProvider.DecryptString($EncryptedData, $Algorithm, $Certificate)
        }
        else {
            $cert = [System.Security.Cryptography.X509Certificates.X509Certificate2]::new($CertificatePath)
            $result = $script:CryptoProvider.DecryptString($EncryptedData, $Algorithm, $cert)
        }
